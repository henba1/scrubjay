...
Topologically this is hub-and-spoke, or *centralized peer-to-peer*: the storage node is the central entity necessary to provide the service. Records travel directly from each peer in the scrubjay network to the central NAS bearing node with no third party in between. If you are concerned about downtime, reliability and the maintenance that self-hosted infrastructure entails, the network drive holding the scrubjay storage can also be located on a cloud VPS.

## Feature list

**Basic.** Transcripts are indexed, can be searched, and can be pulled into context via an [MCP server](https://henba1.github.io/scrubjay/archive-mcp/index.md) running on the NAS host.
