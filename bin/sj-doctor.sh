...
#
#   usage: bin/sj-doctor.sh [--quiet]
#   exit:  0 = everything checked is healthy, 1 = at least one FAIL
#
...
# ── config ─────────────────────────────────────────────────────────────────────────────────────
head_ "config"
host="$(sj_host)"; backend="${SCRUBJAY_TRANSCRIPT_BACKEND:-off}"
...
printf '\n'
if [ "$_fail" -eq 0 ]; then
  printf '\033[32m✓ healthy\033[0m — %d checks passed, %d informational\n' "$_pass" "$_note"
else
  printf '\033[31m✗ %d problem(s)\033[0m — %d checks passed, %d informational\n' "$_fail" "$_pass" "$_note"
fi
[ "$_fail" -eq 0 ]
