---
description: dotclaude — semantically recall a past session/plan/memory across all machines
argument-hint: <topic description> [host=… project=… since=YYYY-MM-DD]
allowed-tools: mcp__dcmcp__dc_recall, mcp__dcmcp__dc_get, mcp__dcmcp__dc_status
---
The user wants to find a past conversation, plan, or memory from the dotclaude archive by
describing its topic — not its filename or which machine it was on.

Topic / filters: **$ARGUMENTS**

1. Call `dc_recall` with the topic as `query` (pass `host` / `project` / `since` only if the user
   gave them). If it reports no archive reachable, say so (run `dc_status` to explain which trees
...
3. Ask which one to pull in (or if the top hit is clearly the one, offer it). On the user's pick,
   call `dc_get` to inject it. For a long transcript, prefer a `turns=` or `lines=` slice around
