...
| `/sjresume [sid8 \| words]` | — | **Continue a session from another machine here** ([hand-off](handoff.md)). Stages the archived transcript — plus its subagents, tasks and file history — into this machine's `~/.claude/projects/`, rewriting the other host's paths to this one's, so Claude Code's own `/resume` can pick it up. Run it from the project dir; then `/resume`. |
...
| `/sjdoctor` | — | **Check that this machine's sync actually works** (runs `bin/sj-doctor.sh`). Verifies config, the data repo, the memory clone, relay reachability, adapters and the last recorded outcomes. Read-only and safe to run any time — reach for it when something *should* have synced and didn't, or before trusting a machine you haven't used in a while. |
