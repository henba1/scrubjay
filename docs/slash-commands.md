...
**app** (not your private data repo), every machine that installs dotclaude gets them out of the
box. They fall into two groups: **archive/recall** commands are thin wrappers that drive the
read-only [`dcmcp` MCP tools](archive-mcp.md); **lifecycle** commands just run the same scripts the
