...
| `/sjbrowse [transcript\|plan\|memory\|note] [host= project= since=]` | `sj_list` → `sj_get` | Eyeballing a **date-sorted list** and grabbing one, when you have no search term in mind — "show me the last few plans on `laptop`." |
