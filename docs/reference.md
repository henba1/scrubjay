...
Paths assume the default `~/.dotclaude/` layout from [Onboarding](onboarding.md) (if you cloned
...
Set in `~/.config/dotclaude/config` (persistent) or inline before a command (one-off) — no file
editing needed:
