...
dotclaude is the **app** — public, and you run it straight from upstream. **You don't fork it.**
...
The onboarder asks which GitHub account owns them (preset it with `DOTCLAUDE_OWNER`) and creates
...
!!! note "Why the app's origin doesn't decide where your data lives"
    ...
    reach for the *maintainer's* private `dotclaude-data`. It refuses to treat the upstream account
...
Claude reads [`AGENTS.md`](https://github.com/henba1/dotclaude/blob/main/AGENTS.md) at
...
generates the dedicated relay SSH key, adds the `claude-receiver` ssh-alias, and prints
the exact `authorized_keys` line to paste on the receiver. It's re-runnable and every
prompt has a default (or preset it via env, e.g. `DOTCLAUDE_OWNER`, `DOTCLAUDE_BACKEND`):
...
Run `bin/dc-bootstrap.sh` on its own to create/seed the private repos without onboarding, and
...
Moving the machinery (e.g. out of `~/code` into `~/.dotclaude`) is machine-local — nothing
to commit. See the [Reference cheatsheet](reference.md#re-home-the-dotclaude-clones).
