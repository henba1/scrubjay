...
`scrubjay-chats` repo is already a complete, permanent home for your transcripts — you don't need
...
bridges the two: it pulls the `scrubjay-chats` repo every 30 min and mirrors it into
`$NAS1/scrubjay-storage`.
...
2. Clone the app for the script:
   ...
   ```
3. Make sure the NAS is mounted (e.g. `/mnt/nas1/scrubjay-storage`).
4. Test once (`CHATS_REPO_URL` is only needed the first run, to clone `scrubjay-chats`):
   ```sh
...
`scrubjay-storage/<host>/<project-slug>/<session>.jsonl` within 30 min of a session
